function createScatterPlot(dataContainer) {
  ...
  svg.append("g").attr("transform", "translate(" + margin + "," + (height + margin) + ")").call(d3.axisBottom(x))
  ...
  function showScatterPlot(data, bounds) {
    ...
    update
      ...
      .each((d) => { if (d.deaths < 0) console.log(d.country) })
  ...
  window.addEventListener("load", updateScatterPlot)
